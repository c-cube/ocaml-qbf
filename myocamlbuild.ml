(* OASIS_START *)
(* DO NOT EDIT (digest: e9a07f44366b129670bec6c5d6d8b23b) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml =
       [
          ("qbf", ["src"], []);
          ("qbf-quantor", ["src/quantor"], []);
          ("qbf-depqbf", ["src/depqbf"], []);
          ("qbf-random", ["src/random"], [])
       ];
     lib_c =
       [("qbf-quantor", "src/quantor/", ["src/quantor/quantor_stubs.h"])];
     flags =
       [
          (["oasis_library_qbf_quantor_ccopt"; "compile"],
            [
               (OASISExpr.EBool true,
                 S
                   [
                      ...
                      A "-ccopt";
                      A "-I../libs/quantor-3.2/"
                   ])
            ]);
          (["oasis_library_qbf_quantor_cclib"; "link"],
            [
               (OASISExpr.EBool true,
                 S
                   [
          ...
          (["oasis_library_qbf_quantor_cclib"; "ocamlmklib"; "c"],
            [
               (OASISExpr.EBool true,
                 S [A "-L."; A "-lquantor"; A "-lpicosat"])
            ]);
          (["oasis_library_qbf_depqbf_cclib"; "link"],
            [
               (OASISExpr.EBool true,
                 S [A "-cclib"; A "-force_load /usr/local/lib/libqdpll.a"])
            ]);
          (["oasis_library_qbf_depqbf_cclib"; "ocamlmklib"; "c"],
            [
               (OASISExpr.EBool true,
                 S [A "-force_load /usr/local/lib/libqdpll.a"])
            ])
       ];
     includes =
       [
          ("tests/quantor", ["src"; "src/quantor"]);
          ("tests/depqbf", ["src"; "src/depqbf"]);
          ("src/random", ["src"]);
          ("src/quantor", ["src"]);
          ("src/depqbf", ["src"])
       ]
